        ...
        for SEED in range(N_SEED):
            ...
            for MODEL_TYPE in MODELS:
                ...
                if MODEL_TYPE == "CombinedModelGAT":
                    ...
                    # Ensure train_loader, val_loader, test_loader are always defined
                    train_loader = loader_train
                    val_loader = loader_validation
                ...
                model, epochs = fast_unified_training(model, DATASET, MODEL_TYPE, train_loader, N_EPOCHS, val_loader, CRITERION, optimizer, FACTOR, SEED, verbose=True, device=device, training_mode=training_mode, triplet_loader=triplet_loader)
                perf_results.loc[i,"epochs"] = epochs
        
                # model evaluation
                model.eval()
                target_perf = fast_unified_performance(model, DATASET+"_test", loader_target, MODEL_TYPE, SEED, device, training_mode=training_mode, data_split="test")
